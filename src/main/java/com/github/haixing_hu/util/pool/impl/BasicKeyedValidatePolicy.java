...
/**
 * A basic implementation of the {@link KeyedValidatePolicy}.
 *
 * @author Haixing Hu
 */
@ThreadSafe
public class BasicKeyedValidatePolicy<K, V> implements
    KeyedValidatePolicy<K, V> {

  private static final Logger LOGGER = LoggerFactory.getLogger(BasicKeyedValidatePolicy.class);
  ...
  public BasicKeyedValidatePolicy() {
  ...
  public BasicKeyedValidatePolicy(final ValidateAction onBorrowAction,
  ...
  @Override
  public boolean onBorrow(final KeyedPoolableFactory<K, V> factory,
      final K key, final V value) throws ValidationFailedException {
    return doValidate(onBorrowAction, factory, key, value);
  }

  @Override
  public boolean onRestore(final KeyedPoolableFactory<K, V> factory,
      final K key, final V value) throws ValidationFailedException {
    return doValidate(onRestoreAction, factory, key, value);
  }

  @Override
  public boolean onCheckIdle(final KeyedPoolableFactory<K, V> factory,
      final K key, final V value) throws ValidationFailedException {
    return doValidate(onCheckIdleAction, factory, key, value);
  }

  protected boolean doValidate(final ValidateAction action,
      final KeyedPoolableFactory<K, V> factory, final K key, final V value)
      throws ValidationFailedException {
    if (action == NOT_VALIDATE) {
      return true;
    }
    if (factory.validate(key, value)) {
      return true;
    }
    // the obj did not pass the validation
    switch (action) {
      case THROWN_ON_FAILED: {
        final ValidationFailedException e = new ValidationFailedException(key,
            value);
        factory.destroy(key, value);
        throw e;
      }
      case LOG_ON_FAILED: {
        LOGGER.error("Validation of object failed: key = {}, value = {}", key,
            value);
        factory.destroy(key, value);
        return false;
      }
