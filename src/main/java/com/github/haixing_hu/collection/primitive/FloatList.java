...
/**
 * An ordered collection of {@code float} values.
 *
 * @author Haixing Hu
 */
public interface FloatList extends FloatCollection {
  ...
  /**
   * Returns {@code true} iff <i>that</i> is an {@code FloatList} that
   * contains the same elements in the same order as me. In other words, returns
   * {@code true} iff <i>that</i> is a {@code FloatList} that has the
   * same {@link #size() size} as me, and for which the elements returned by its
   * {@link FloatList#iterator iterator} are equal ({@code ==}) to the
   ...
   * {@code FloatList} interface.)
   *
   * @param that
   *          the object to compare to me
   * @return {@code true} iff <i>that</i> is an {@code FloatList} that
   ...
   * <p />
   * The hash code of an {@code FloatList} is defined to be the result of
   ...
   * {@code FloatList} is backed by me, so that any changes in the returned
