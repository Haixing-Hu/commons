...
public interface LongList extends LongCollection {
   ...
   * returned by the given collection's {@link LongCollection#iterator iterator}
   * .
   *
   * @param current
  ...
  /**
   * Returns <code>true</code> iff <i>that</i> is an <code>LongList</code> that
   * contains the same elements in the same order as me. In other words, returns
   * <code>true</code> iff <i>that</i> is a <code>LongList</code> that has the
   * same {@link #size() size} as me, and for which the elements returned by its
   * {@link LongList#iterator iterator} are equal (<code>==</code>) to the
   ...
   * <code>LongList</code> interface.)
   *
   * @param that
   *          the object to compare to me
   * @return <code>true</code> iff <i>that</i> is an <code>LongList</code> that
   ...
   * <p />
   * The hash code of an <code>LongList</code> is defined to be the result of
   ...
   * {@link java.util.List List} of {@link Long}s.
   *
   * @return my hash code
   */
  @Override
  ...
  /**
   * Returns an {@link LongIterator iterator} over all my elements, in the
   ...
   * <code>LongList</code> is backed by me, so that any changes in the returned
