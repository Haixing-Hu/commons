...
public interface LongList extends LongCollection {
  ...
  /**
   * Returns {@code true} iff <i>that</i> is an {@code LongList} that
   * contains the same elements in the same order as me. In other words, returns
   * {@code true} iff <i>that</i> is a {@code LongList} that has the
   * same {@link #size() size} as me, and for which the elements returned by its
   * {@link LongList#iterator iterator} are equal ({@code ==}) to the
   ...
   * {@code LongList} interface.)
   *
   * @param that
   *          the object to compare to me
   * @return {@code true} iff <i>that</i> is an {@code LongList} that
   ...
   * <p />
   * The hash code of an {@code LongList} is defined to be the result of
   ...
   * {@code LongList} is backed by me, so that any changes in the returned
