...
public interface CharList extends CharCollection {
  ...
  /**
   * Returns <code>true</code> iff <i>that</i> is an <code>CharList</code> that
   * contains the same elements in the same order as me. In other words, returns
   * <code>true</code> iff <i>that</i> is a <code>CharList</code> that has the
   * same {@link #size() size} as me, and for which the elements returned by its
   * {@link CharList#iterator iterator} are equal (<code>==</code>) to the
   ...
   * <code>CharList</code> interface.)
   *
   * @param that
   *          the object to compare to me
   * @return <code>true</code> iff <i>that</i> is an <code>CharList</code> that
   ...
   * <p />
   * The hash code of an <code>CharList</code> is defined to be the result of
   ...
   * <code>CharList</code> is backed by me, so that any changes in the returned
