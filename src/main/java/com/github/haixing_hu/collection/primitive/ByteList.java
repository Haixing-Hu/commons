...
public interface ByteList extends ByteCollection {
  ...
  /**
   * Returns {@code true} iff <i>that</i> is an {@code ByteList} that
   * contains the same elements in the same order as me. In other words, returns
   * {@code true} iff <i>that</i> is a {@code ByteList} that has the
   * same {@link #size() size} as me, and for which the elements returned by its
   * {@link ByteList#iterator iterator} are equal ({@code ==}) to the
   ...
   * {@code ByteList} interface.)
   *
   * @param that
   *          the object to compare to me
   * @return {@code true} iff <i>that</i> is an {@code ByteList} that
   ...
   * <p />
   * The hash code of an {@code ByteList} is defined to be the result of
   ...
   * {@code ByteList} is backed by me, so that any changes in the returned
